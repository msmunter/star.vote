...
class UserController extends Controller
{
	...
	public function init()
	{
		...
		if ($this->userID) {
			...
			//$this->person = $this->model->getPersonByUserID($this->userID);
			//$this->org = $this->model->getOrgByUserID($this->userID);
		}
	}
	
	// View single user
	public function details() {
		// Get by SeoID if one exists
	...
	public function logout()
	{
		...
		// Destroy cookies
		setcookie("authToken", "", $dateTime->format('U'), '/', '.greenhealtheugene.com');
		setcookie("authID", "", $dateTime->format('U'), '/', '.greenhealtheugene.com');
		// Send to home page
		header("Location: /");
	}
	...
	public function dologin()
	{
		...
		if ($_POST['email'] && $_POST['pass']) {
			...
			if ($this->userID) {
				...
				if ($_POST['authLength'] > 0) {
					setcookie("authToken", $authToken, $seObject->format('U'), '/', '.greenhealtheugene.com');
					setcookie("authID", $this->userID, $seObject->format('U'), '/', '.greenhealtheugene.com');
				} else {

					...
					if (($this->survey->verifiedVoting && $voterKeyResult->surveyID) || $this->survey->verifiedVoting == false) {
						...
						if ($oNow >= $oStart && ($oNow < $oEnd || empty($oEnd))) {
							...
							if (empty($voterKeyResult->voteTime)) {
								...
								if (!empty($this->yourVotes) || $tempVote != false || $voterIdent->verificationState != 'new') {
									$this->hasVoted = true;
			...
			if ($this->URLdata != '') {
				if (strlen($this->URLdata) < 8 || strlen($this->URLdata) > 8) {
					...
					if ($this->survey) {
						...
						$this->toBeProcessedVoters = $this->model->getToBeProcessedVoters($this->URLdata);
						$this->resultsVoterCount = $this->model->getResultsVoterCount($this->URLdata);
						$this->percentReporting = round($this->resultsVoterCount / ($this->tempVoterCount - $this->rejectedTwiceVoterCount) * 100, 1);
					}
				}
			} else {
